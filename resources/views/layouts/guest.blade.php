...
<head>
    ...
    <title>{{ config('app.name', 'Onscholarship') }} - @yield('title', 'Connecting Africa to Quality Education in China')</title>
    <meta name="description" content="@yield('meta_description', 'Connecting Africa to Quality Education in China. Providing comprehensive admission processing, scholarship placement, and visa assistance.')">
    <meta name="keywords" content="@yield('meta_keywords', 'Onscholarship, Study in China, Scholarships, International Students')">
    <meta property="og:type" content="website">
    <meta property="og:title" content="{{ config('app.name', 'Onscholarship') }} - @yield('title', 'Welcome')">
    <meta property="og:description" content="@yield('meta_description', 'Connecting Africa to Quality Education in China.')">
    @hasSection('og_image')
    <meta property="og:image" content="@yield('og_image')">@endif
...
<body class="font-sans antialiased bg-white dark:bg-[#0b1120] text-gray-900 dark:text-gray-100 transition-colors duration-300"
        ...
        <div class="max-w-7xl mx-auto rounded-3xl transition-all duration-500 border"
            ...
            <div class="flex items-center justify-between gap-4">
                ...
                <a href="{{ route('home') }}" class="flex items-center flex-shrink-0">
                    <img src="{{ asset('images/logo.png') }}" alt="Onscholarship" class="h-10 w-auto object-contain" style="filter: brightness(0) invert(1);">
                </a>
                ...
                <nav class="hidden lg:flex items-center gap-8 font-medium text-sm flex-1 justify-center whitespace-nowrap">
                    ...
                    <a href="{{ route($link['route']) }}"
                        class="nav-link transition-colors pb-0.5 {{ request()->routeIs($link['match']) ? 'text-white font-bold active' : 'text-white/65 hover:text-white' }}">
                        {{ $link['label'] }}
                    </a>
                    @endforeach
                </nav>
                ...
                <div class="hidden md:flex items-center gap-3 flex-shrink-0">
                    ...
                    <!-- Login / CTA -->
                    @auth
                    <a href="{{ route('dashboard') }}" class="font-bold text-[13px] uppercase tracking-wide text-white rounded-full px-5 py-2 hover:opacity-90 transition flex-shrink-0" style="background:#0D47A1; box-shadow:0 4px 10px rgba(13,71,161,.2);">Dashboard</a>
                    @else
                    <a href="{{ route('login') }}" class="font-bold text-sm text-white/70 hover:text-white transition px-2">Log in</a>
                    <a href="{{ route('register') }}" class="font-bold text-[13px] uppercase tracking-wide text-white rounded-full px-5 py-2 hover:opacity-90 transition flex-shrink-0" style="background:#1FA463; box-shadow:0 4px 10px rgba(31,164,99,.25);">Apply Now</a>
                    @endauth
                </div>
    ...
    <div class="mobile-menu" :class="{ 'open': mobileOpen, 'bg-[#0f172a]': $store.theme.dark, 'bg-white': !$store.theme.dark }" role="dialog" aria-modal="true">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-5 space-y-1">
            @foreach($navLinks ?? [['route'=>'home','label'=>'Home','match'=>'home'],['route'=>'universities.index','label'=>'Universities','match'=>'universities.*'],['route'=>'programs.index','label'=>'Programs','match'=>'programs.*'],['route'=>'scholarships.index','label'=>'Scholarships','match'=>'scholarships.*'],['route'=>'services','label'=>'Services','match'=>'services'],['route'=>'about','label'=>'About Us','match'=>'about'],['route'=>'blog.index','label'=>'Blog','match'=>'blog.*'],['route'=>'contact','label'=>'Contact','match'=>'contact']] as $link)
            <a href="{{ route($link['route']) }}"
    ...
    <footer style="background:#0f172a;" class="relative z-10 text-white pt-20 pb-8 border-t-4 border-[#1FA463]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-14">
                ...
                <div class="lg:col-span-1">
                    <a href="{{ route('home') }}" class="inline-flex items-center mb-5">
                        <img src="{{ asset('images/logo.png') }}" alt="Onscholarship" class="h-12 w-auto object-contain" style="filter: brightness(0) invert(1);">
                    </a>
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-sm">
                        @foreach([['home','Home'],['about','About Us'],['universities.index','Universities'],['scholarships.index','Scholarships'],['programs.index','Programs']] as [$r,$l])
                        <li><a href="{{ route($r) }}" class="text-gray-400 hover:text-white hover:translate-x-1 inline-flex items-center gap-2 transition-all group">
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-sm">
                        @foreach(['Admission Processing','Scholarship Placement','Visa Assistance','Document Translation','Agent Partnership'] as $svc)
                        <li><a href="{{ route('services') }}" class="text-gray-400 hover:text-white hover:translate-x-1 inline-flex items-center gap-2 transition-all group">
                                <svg class="w-3 h-3 text-[#1FA463] opacity-0 group-hover:opacity-100 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7" />
                                </svg>
                                {{ $svc }}
                ...
                <div>
                    ...
                    <ul class="space-y-4 text-sm text-gray-400">
                        ...
                        <li class="flex items-start gap-3">
                            ...
                            <a href="mailto:info@onscholarship.com" class="hover:text-white transition">info@onscholarship.com</a>
                        </li>
                        <li class="flex items-start gap-3">
                            <div class="w-8 h-8 rounded-lg bg-white/5 border border-gray-700 flex items-center justify-center flex-shrink-0 mt-0.5" style="color:#1FA463;">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="border-t border-gray-800 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-gray-500">
                <p>&copy; {{ date('Y') }} Onscholarship Co., Ltd. All rights reserved.</p>
