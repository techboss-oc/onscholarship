...
@section('title', 'My Wishlist | Onscholarship')
...
<div class="p-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <a href="{{ route('student.programs.search') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-[#f15a24] text-white rounded-lg hover:bg-[#d94e1c] transition text-sm font-medium">
    ...
    @if($wishlist->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-12 text-center">
            ...
            <a href="{{ route('student.programs.search') }}" class="text-[#f15a24] font-medium hover:underline">Find Programs Now &rarr;</a>
        </div>
    @else
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-all duration-300 group">
                    <div class="relative h-48">
                        @if($program->university->logo)
                            <img src="{{ Storage::url($program->university->logo) }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-gradient-to-br from-[#0f2441] to-[#1a3a5f] flex items-center justify-center text-white/20 text-4xl font-bold italic">
                                {{ substr($program->university->name, 0, 1) }}
                            </div>
                        @endif
                        <div class="absolute top-4 right-4">
                            <form action="{{ route('student.wishlist.toggle') }}" method="POST">
                                @csrf
                    ...
                    <div class="p-5">
                        ...
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white line-clamp-1 mb-1">{{ $program->name }}</h3>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-50 dark:border-gray-700">
                            <div>
                                ...
                                <p class="text-sm font-bold text-[#f15a24]">${{ number_format($program->tuition_fee, 0) }}/yr</p>
                            </div>
                            <a href="{{ route('student.applications.create', ['program_id' => $program->id]) }}" class="px-4 py-2 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-lg text-sm font-bold hover:bg-gray-800 transition">
                                Apply Now
                            </a>
                        </div>
                    </div>
