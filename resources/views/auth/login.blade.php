<x-guest-layout>
    {{-- Login Page Background Enhancements  --}}
    <div class="fixed inset-0 pointer-events-none" style="z-index:-1;">
        <div class="absolute inset-0" style="background: {{ $type === 'agent' ? 'linear-gradient(135deg, #f0fdf4 0%, #e8f5ee 50%, #ffffff 100%)' : 'linear-gradient(135deg, #f0f7ff 0%, #e8f0fe 50%, #ffffff 100%)' }};"></div>
        <div class="absolute inset-0 dark:bg-gray-900"></div>
        {{-- Floating background blobs to make glassmorphism pop --}}
        <div class="absolute top-[20%] left-[10%] w-[400px] h-[400px] rounded-full blur-[100px] opacity-40 mix-blend-multiply dark:mix-blend-screen" style="background: {{ $type === 'agent' ? '#1fa463' : '#3b82f6' }};"></div>
        <div class="absolute bottom-[10%] right-[10%] w-[500px] h-[500px] rounded-full blur-[120px] opacity-30 mix-blend-multiply dark:mix-blend-screen" style="background: {{ $type === 'agent' ? '#86efac' : '#8b5cf6' }};"></div>
    </div>
    ...
    <div class="min-h-screen flex items-center justify-center py-24 px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="max-w-md w-full bg-white/60 dark:bg-[#1e293b]/70 backdrop-blur-2xl rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-white/50 dark:border-gray-700/50 overflow-hidden relative">
            ...
            <div class="h-2 w-full absolute top-0 left-0" style="background: {{ $type === 'agent' ? 'linear-gradient(90deg, #1FA463, #34d399)' : 'linear-gradient(90deg, #0b49b3, #3b82f6)' }}; shadow-md"></div>
            ...
            <div class="p-8 sm:p-10 relative z-10">
                <div class="mb-8 text-center">
                    ...
                    <div class="flex justify-center mb-4">
                        <div class="w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg" style="background: {{ $type === 'agent' ? 'linear-gradient(135deg, #1FA463, #15804c)' : 'linear-gradient(135deg, #0b49b3, #0D47A1)' }};">
                            @if($type === 'agent')
                    ...
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                        {{ $type === 'agent' ? 'Enter your agent credentials securely.' : 'Dive into your admission dashboard.' }}
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" class="block w-full px-4 py-3 rounded-xl border border-gray-300/50 dark:border-gray-600/50 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm text-gray-900 dark:text-white focus:ring-2 focus:border-transparent transition-all shadow-inner" style="--tw-ring-color: {{ $type === 'agent' ? '#1fa463' : '#0b49b3' }}">
                    ...
                    <div class="flex items-center justify-between mt-6">
                        ...
                        @if (Route::has('password.request'))
                            <a class="text-sm font-bold opacity-80 hover:opacity-100 transition-opacity" style="color: {{ $type === 'agent' ? '#1fa463' : '#0b49b3' }}" href="{{ route('password.request') }}">
                                Forgot Password?
                            </a>
                        @endif
                    </div>
