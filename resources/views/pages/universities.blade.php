...
@section('meta_description', 'Discover top universities in China for international students. Explore partner universities with Onscholarship.')
@section('meta_keywords', 'Universities in China, Study in China, Partner Universities, Onscholarship')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="group bg-white dark:bg-gray-800 rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 flex flex-col">
                ...
                <div class="p-6 flex-grow flex flex-col">
                    <h4 class="text-xl font-bold text-gray-900 dark:text-white mb-2 brand-font line-clamp-1">{{ $university->name }}</h4>
                    <p class="text-gray-500 dark:text-gray-400 text-sm mb-4 line-clamp-2">{{ Str::limit($university->description ?? 'Discover exclusive programs and opportunities offered at this esteemed institution.', 80) }}</p>

                    <div class="mt-auto border-t border-gray-100 dark:border-gray-700 pt-4 flex justify-between items-center">
                        <span class="font-bold text-[#0f2441] dark:text-gray-200 text-sm">{{ $university->programs_count ?? 0 }} Programs</span>
                        <a href="{{ route('universities.show', $university->id) }}" class="text-[#f15a24] text-sm font-semibold hover:text-[#d94a1c] transition">View Details &rarr;</a>
                    </div>
                </div>
            </div>
            @endforeach
