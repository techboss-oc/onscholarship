...
<head>
    ...
    <title><?php echo e(config('app.name', 'Onscholarship')); ?> - Admin Platform</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .sidebar-logo-text { color: var(--sidebar-logo); }
        .sidebar-logo-accent { color: #f15a24; }
        ...
        .badge-suspended { background: rgba(107,114,128,0.12); color: #6b7280; }
        .badge-active    { background: rgba(16,185,129,0.12); color: #059669; }

        /* Scrollbar */
        ::-webkit-scrollbar { width: 5px; height: 5px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: rgba(15,36,65,0.15); border-radius: 10px; }
...
" :class="{ 'dark': darkMode }">
    ...
    <aside class="admin-sidebar fixed inset-y-0 left-0 z-50 w-64 transition-transform duration-300 ease-in-out"
        ...
        <!-- Logo -->
        <div class="flex items-center h-16 px-6 flex-shrink-0" style="border-bottom: 1px solid var(--sidebar-border);">
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center flex-shrink-0 w-full px-2" style="height: 100%;">
                <img src="<?php echo e(asset('images/logo.png')); ?>" alt="Onscholarship" class="h-8 w-auto object-contain">
            </a>
        </div>
        ...
        <nav class="p-3 flex-1 overflow-y-auto">
            ...
            <a href="<?php echo e(route('admin.agents.index')); ?>" 
               class="sidebar-link <?php echo e(request()->routeIs('admin.agents.*') ? 'active' : ''); ?>">
            ...
            <a href="<?php echo e(route('admin.applications.index')); ?>" 
               class="sidebar-link <?php echo e(request()->routeIs('admin.applications.*') ? 'active' : ''); ?>">
            ...
            <a href="<?php echo e(route('admin.universities.index')); ?>" 
               class="sidebar-link <?php echo e(request()->routeIs('admin.universities.*') ? 'active' : ''); ?>">
                <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m3-4h1m-1 4h1m-5 8h8"/></svg>
                Universities
            </a>
            <a href="<?php echo e(route('admin.programs.index')); ?>" 
               class="sidebar-link <?php echo e(request()->routeIs('admin.programs.*') ? 'active' : ''); ?>">
                <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                Programs
            </a>
            <a href="<?php echo e(route('admin.scholarships.index')); ?>" 
               class="sidebar-link <?php echo e(request()->routeIs('admin.scholarships.*') ? 'active' : ''); ?>">
            ...
            <a href="<?php echo e(route('admin.blog_categories.index')); ?>" 
               class="sidebar-link <?php echo e(request()->routeIs('admin.blog_categories.*') ? 'active' : ''); ?>">
        ...
        <header class="admin-topbar sticky top-0 z-40 h-16 px-4 sm:px-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <button @click="darkMode = !darkMode; localStorage.theme = darkMode ? 'dark' : 'light'; if(darkMode) { document.documentElement.classList.add('dark') } else { document.documentElement.classList.remove('dark') }" 
                        class="p-2 rounded-lg hover:bg-[var(--border)] transition"
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false"
                        ...
                        <div class="hidden md:block text-left">
                            <p class="text-xs font-bold leading-tight" style="color: var(--text-primary);"><?php echo e(explode(' ', auth()->user()->name)[0]); ?></p>
                            <p class="text-[10px] leading-tight" style="color: var(--text-muted);">Administrator</p>
                        </div>
                        <svg class="w-3.5 h-3.5 hidden md:block" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color: var(--text-muted);"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                    </button>
                    
                    <div x-show="open" x-transition.opacity 
                         class="admin-dropdown absolute right-0 mt-2 w-52 z-50 overflow-hidden py-1">
                        <div class="px-4 py-3 border-b" style="border-color: var(--border);">
                            <p class="text-xs font-bold" style="color: var(--text-primary);"><?php echo e(auth()->user()->name); ?></p>
                            <p class="text-xs truncate" style="color: var(--text-muted);"><?php echo e(auth()->user()->email); ?></p>
                        </div>
                        <a href="<?php echo e(route('home')); ?>" target="_blank">View Website</a>
        ...
        <main class="p-4 sm:p-6 lg:p-8 min-h-[calc(100vh-4rem)]">
            ...
            <?php if(session('error')): ?>
                <div class="mb-6 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-xl text-red-700 dark:bg-red-900/20 dark:text-red-300 flex justify-between items-center"
                    ...
                    <button @click="show = false" class="p-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
            <?php endif; ?>
...
<?php /**PATH C:\laragon\www\hainan\resources\views/layouts/admin.blade.php ENDPATH**/ ?>